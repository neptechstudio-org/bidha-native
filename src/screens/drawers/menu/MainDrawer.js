...
renderMainDrawerComponents = mainDrawerItems => Object.keys(mainDrawerItems).map(item => (
  <OuterDrawerItem
    key={item}
    label={item}
    onPress={() => {
  ...
  render() {
    // console.log("props in MainDrawer",this.props.items);
    ...
    console.log('scopedItemsArr', scopedItemsArr);
    return (
      <ScrollView>
        <DrawerHeader navigateToCallback={this.navigateToCallback} />
        <TouchableOpacity
